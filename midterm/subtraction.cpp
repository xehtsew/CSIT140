...
ostream& operator<< (ostream& strm, const Subtraction& obj) {
	...
	void Subtraction::randomize() {
		...
		num1 = (rand() % 10) + 1;
		num2 = (rand() % 10) + 1;
		//if the result is less than 0, get new numbers
		while (num1 - num2 < 0) {
			num1 = (rand() % 10) + 1;
			num2 = (rand() % 10) + 1;
		}
	...
	void Subtraction::validate(string str1) {
		...
		//cast str1 as integer and store in Subtraction::userResponse
